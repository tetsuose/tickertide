...
> **进度（2026-06-06）：M0 ✅ DONE（数据+引擎，PR #1–#8）· M1 ✅ DONE（Web + Discovery + C9，PR #9–#14）· M2 ✅ DONE（Ocean canvas，PR #16–#20）· M3 ✅ DONE（Rotation：sector ETF→bucket_bars→RS-Ratio/league→rotation.json→SVG 多线+钻进+第 2 scope writer，PR #25–#29）· M4 ⏳ NEXT（主题分类）。**
...
## D — 部署 + 夜间自动化（infra 轨）★ 详细方案 ⏳ 建议 M3 后启动
...
**目标**：把静态客户端 + nightly 真实数据管道接通并私有上线（Cloudflare Pages），让工具天天可用、并把最高风险的真实 ingest + 自动化逼到现在暴露（而非拖到 M6）。访问控制与数据源条款是两个独立 gate，后者按 PRD §6.2 / NFR-7 复核。

**技术栈/范式（PRD §5.1/§5.2/§16）**：离线预计算 + 静态分发；GitHub Actions（cron）跑 `pipeline→export→web-build→发布`；Cloudflare Pages 私有 host（WORKFLOW §8 已点名）。`engine/index.py` 仍 stdlib-only；CI 仅给产品代码装 `requirements.txt`。
...
| **D.3** 静态 host 上线（私有） | `chore/deploy-pages` | Cloudflare Pages 接 D.2 产物（nightly 发布 dist）；**私有访问**（Cloudflare Access / 私有项目）；auto-deploy 接线 | host 配置（wrangler/pages）+ deploy workflow | 站点私有可访问、Vite `base='./'` 路径正常、数据 fetch 正常、nightly 刷新可见 |
...
- **访问控制**：host 默认私有，Cloudflare Access 限白名单；数据源条款复核另按 PRD §6.2 / NFR-7。
...
### AC-D（部署轨完成）
真实数据端到端（D.1：AC-M0 真实过）；nightly Action 自动刷新（D.2）；**Cloudflare Pages 私有上线、访问到最新 as_of 的 Discovery/Ocean/Rotation**（D.3）；无 secrets 入库；陈旧/失败可见（D.4）。
...
| **M4.5** LLM 抽取 + human 审批 | `feat/theme-llm-review` | `extract.py`(LLM 读 EDGAR business/segment → candidate exposure=revenue_share；segment 脏兜底 C6) + `review.py`(human 审批 candidate→theme_membership、写 approved_by) + `run.py`(编排) | `themes/extract.py` `themes/review.py` `themes/run.py` | candidate→approve 流；approved_by 写入；缺 segment 标低置信不宣称精确(C6) |
| **M4.6** 验证 + 构建 | `feat/theme-pipeline-build` | AC-M4 端到端 + Makefile(themes/theme-index/theme rotation/theme-c9 接线) + 测试 + 浏览器 smoke | `Makefile` `web/src/views/Rotation.test.tsx`(theme) `themes/*` 测试 | AC-M4 全过；PIT 不回溯 + 非市值加权 + theme 多线头测 |

> 顺序：M4.1(membership)→M4.2(index)→M4.3(RS-Ratio)→M4.4(export+web)→M4.5(LLM/human 真实路径)→M4.6(收口)。**主链(M4.1–4)用 seed/fixture membership 确定性验证**；M4.5 LLM 抽取需 API/EDGAR/human(本环境难自动验证) → 框架 + 流程，真实数据时用(同 M3 真实 ingest 兜底)。
