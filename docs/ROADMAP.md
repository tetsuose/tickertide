...
## M5 — Valuation screener + Stock detail（纲要）★ ⏳ 预览已上线（#53），正式版待开

- **预览（#53，board.json 驱动，C9 同源）**：两个空 tab 已上线——Valuation 横截面可排序表（as-of 三档上色 + common-vintage pctile[fresh cohort 内排、stale 显 vint] + scope 下拉=第 3 个全局 writer）；Stock per-name 面板（不受 scope · ticker 下拉 · 头部 theme chips 带 exposure% · 价格/MA/成交量 MiniChart · 6 估值卡 · 5 分量）。`web/src/lib/format.ts` 抽公共格式化；12 SSR 测试。
- **目标（正式版）**：duckdb-wasm 浏览器全 universe 横截面查询 + PEG/margin；Stock 的 price↔fundamentals 时间轴对齐 stack（季度营收 bars + 每日 P/S over time，共用 x 轴）+ 最新 filing AI 摘要。
- **前置**：M0 估值；`export/` 的 Parquet 分片 + per-name bundle（含季度营收 + 每日 P/S 序列）。
- **关键未决**：duckdb-wasm 接法、Parquet 分片粒度、Stock per-name bundle 结构（PRD §17）。
- **验收**：PRD §14 AC-M5。
