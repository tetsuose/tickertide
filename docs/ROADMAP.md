...
> ⚠️ **已被 M8 取代（SUPERSEDED）**：M2 的 **RS percentile × Valuation percentile 周度散点**（含象限/十字线/WEEK scrubber/pin trail）在 **M8** 重构为 **Ignition × Valuation 日频「海平面图」**（y=`ign_pct`、海平面 90、x=log raw P/S、日期滑杆 + Play 插值动画）。本节保留作历史记录，**现行 Ocean 设计见下方「M8 — Ocean Rebuild」段 + PRD §9.2**。
...
## M5 — Valuation screener + Stock detail ★ 详细方案 ✅ DONE（M5.1–5.5，PR #57–#61；wasm 走 R2，待 operator 配）
...
| task | 分支建议 | 范围 | 产出文件 | 验收 |
|---|---|---|---|---|
| **M5.1 ✅ #57** 估值横截面 Parquet 导出 | `feat/m5-1-valuation-parquet` | 全 universe 最新 valuation 横截面（+peg/margin/freshness）→ `valuation.parquet` + `.meta.json`；接 Makefile export | `export/valuation_parquet.py` `Makefile` | parquet 全 universe 行、含 peg/margin 列、freshness 三档；与 board.json 同 valuation_daily（C9）；nightly 产出 |
...
## M7 — ignition 发现引擎 + Discovery 持续点火榜 ★ 详细方案 ✅ DONE（M7.1–M7.5，PR #64–#67 / #68）

> **状态：✅ DONE（2026-06-13，M7.1–M7.5 全 merged）。** 实证立项 → 规格 PRD §10.8 / BUILD-PLAN §4.8 → 五个 task 端到端落地：M7.1 ignition compute（#64）→ M7.2 持续点火榜 export（#65）→ M7.3 Discovery 持续点火榜（#66）→ M7.4 Stock 点火诊断（#67）→ M7.5 验证 + 收口（#68：`make ac-m7` / `ac-m7-e2e` 一键复验 AC-M7 五条）。第二台 per-stock 引擎已与 composite 并列，Discovery 改「持续点火」排序（非 composite），双引擎同源 C9。
...
| **M7.5 ✅ #68** 验证 + 收口 | `feat/m7-build` | AC-M7 五条聚合为单一带标签门禁 `make ac-m7`（+ `ac-m7-e2e` 一命令端到端）+ 收口文档 | `compute/check_ac_m7.py`（新）·`Makefile` | AC-M7 五条全 PASS、GATE_PASS、一键复验 |
...
## M8 — Ocean Rebuild: Ignition Sea-Level Animation ✅ DONE

> **状态：✅ DONE（2026-06-15，分支 `feat/m8-ocean-ignition-sea-level`）。** 把 Ocean 从「RS percentile × Valuation percentile 周度散点」(M2) 重构为「**Ignition × Valuation 日频海平面图**」+ 日期滑杆 / Play 插值动画；同步把 **composite 从全部用户可见层撤下**（Discovery / Stock / Rotation / 顶栏），脊柱收敛为 **ignition core + valuation + raw evidence**。M2 散点已 SUPERSEDED（见 M2 段头注 + PRD §9.2）。

**目标**：让 Ocean 与脊柱核心（ignition 持续点火）对齐 —— 一个点浮在「海平面」之上即是 Discovery 的持续点火 candidate（同一道 gate，C9），横轴是可直接读的**原始估值**（log P/S，无隐含阈值参数）。用时间动画取代旧的 pinned 多周尾迹，让「点火→冒头」的时间演化一眼可见。

**前置**：M7（ignition 引擎 + `derived_daily` 的 `ign_pct`/`ign_persist_days`）+ M0 估值（`valuation_daily`）。**对应模块**：`export/`（`ocean.py` 重写）+ `web/`（`ocean-draw.ts` + `Ocean.tsx`）+ `compute/`/`export/`（Rotation 聚合改 ignition）。

### M8 改了什么（凝练）

- **轴重定义**：
  - y = `ign_pct`（0–100，§10.8 的 ignition 横截面百分位）；固定**海平面线 @ `ign_pct=90`**（top decile / 点亮阈值）。在海平面之上 = 正在点火（加速 / 突破 / 放量），冒头高度 ∝ `ign_pct − 90`。
  - x = **原始 trailing P/S TTM，log 刻度**（**非估值百分位、非任何 composite/混合估值分、无隐含阈值参数**）；`x_domain` 由数据计算。
  - **candidate = `ign_pct≥90` AND `ign_persist_days≥5`** —— 与 Discovery 排序所用**同一道持续点火 gate**（海平面之上的点 = Discovery candidate；C9）。
- **日期滑杆 + Play 插值动画**：滑杆 scrub EOD 快照（默认开在最新 EOD）；Play/Pause + 上一/下一帧；Play 用 requestAnimationFrame 在**相邻真实 EOD 快照**间平滑 tween 点位（每段 900–1200ms，到最新自动停）。**插值仅视觉**——tooltip / 状态永远读真实快照，绝不读窗口内伪造的中间值；拖动滑杆即暂停 + 重置 tween。
- **导出窗口**：默认最近 **60 个交易日的日频快照**（原为 14 个周度快照）。
- **tooltip**：ticker、sector、`ign_pct`、`ign_persist_days`、candidate、P/S、EV/S、P/E、EV/EBITDA、10d/1m return、放量 `vol_mult`、估值新鲜度。
- **保留**：sector/theme 配色、scope filter、click→pin、lasso→set 全局 scope（Ocean 仍是 scope 的**第一个 writer**，C10）。**砍掉**：旧的象限配色模式、(50,50) 十字线、周度 WEEK scrubber、pinned 多周尾迹线/箭头（Play 动画替代时间尾迹）。

### composite 退出用户可见层（M8 一并完成）

- **Discovery 卡（EvidenceCard）**：去掉 composite 角标、composite 日环比 delta、可展开 5 分量面板。卡片 ignition-first（🔥 candidate 徽章或暗淡 `ign_pct` chip + 点火证据 strip + 6 个原始证据数 + mini 图）。
- **Stock 页**：headline 改 `ign_pct`（标签「IGN PCT · 发现核心」），candidate 时变绿；composite 5 分量条 stack 移除；ignition 诊断面板保留。
- **Rotation league**：「composite 中位（`composite_median`）」统计改为 ignition 聚合「**# igniting**」（成员 `ign_pct≥90`）+「**# candidates**」（持续点火 gate）；成员预览表头「top by composite」→「**top by 持续点火**」。
