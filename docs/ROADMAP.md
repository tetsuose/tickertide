...
**目标**：把静态客户端 + nightly 真实数据管道接通并私有上线（Cloudflare Pages），让工具天天可用、并把最高风险的真实 ingest + 自动化逼到现在暴露（而非拖到 M6）。**个人自用、私有、不对外分发数据（SCOPE / §6.2 / NFR-7 硬线）。**
...
- **不分发数据**（SCOPE / §6.2 / NFR-7）：host 私有、只服务个人。
