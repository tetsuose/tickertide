...
> **进度（2026-06-12）：M0 ✅ DONE（数据+引擎，PR #1–#8）· M1 ✅ DONE（Web + Discovery + C9，PR #9–#14）· M2 ✅ DONE（Ocean canvas，PR #16–#20）· M3 ✅ DONE（Rotation，PR #25–#29）· D ✅ DONE（D.1–D.4：真实 ingest→nightly→Cloudflare Pages tickertide.pages.dev→新鲜度 badge，PR #37–#44、#48、#50）· M4 ⏳ 主链+收口 DONE（M4.1–4.4 PR #32–#36；M4.6 pipeline/Makefile 接线 + AC-M4 check），仅剩 M4.5（LLM 抽取 + human 审批）· 之后 M5（Valuation screener + Stock detail）。**
...
## M4 — 主题分类（concept themes）★ 详细方案 ⏳ M4.1–4.4 + M4.6 DONE · 剩 M4.5
