...
**P7 — 双引擎互补，不混为旋钮。** composite 答「是否已成型 leader」（守已成），ignition 答「是否刚开始加速」（抓刚起）；两者是不同物理量。early⟷reliable 旋钮**只属 composite**；early 发现的精度靠 persistence（§10.8），不靠把旋钮推到 early。
...
- **候选池 = 三级漏斗前两级**（§10.8）：① ignition 跨入横截面 top decile（触发，recall）→ ② persistence 连续 ~5 日仍在（去假突破，把 forward LIFT 从 ≈0 提到正，实证 `analysis/precision_ignition.py`）→ 上榜；③ 用户翻财报 = 终筛（human，§10.8）。bound = 「持续点火」名单而非硬 top-20；旋钮调候选池松紧。
...
> **为什么是第二台引擎、不是 composite 的旋钮（已定，实证支撑）：** composite（§10.6）是趋势**确认**引擎——5 分量全是长窗口 / 测水平（rs 63/126、high 252、trend KER63、vol 50/200），数学上系统性滞后于「早期」。`early⟷reliable` 旋钮只在这些**都滞后**的分量间重配权、从不引入短窗口信号，故 k=1「early」仍滞后。早期发现与趋势确认测的是**不同的物理量**（加速度 / 拐点 vs 水平），不是一个标量的两端 → 必须是并列的第二台引擎。
...
- **ignition 引擎（§10.8，2026-06-13 立项）**：早期发现 = 第二台引擎，5 个短窗口分量（加速 / 收缩-扩张 / 放量 / 突破收复 / RS 拐点）→ 横截面 top decile；**瞬时点火无精度，唯 persistence（持续 ~5 日）有 lift**（实证 `analysis/precision_ignition.py`）→ **Discovery = 「持续点火」榜**（非 composite 排序）；三级漏斗 = 触发 → 持续 → 翻财报。early⟷reliable 旋钮属 composite，不混入 ignition。
