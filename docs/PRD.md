...
- **无常驻 server、无托管 DB、不向 client 暴露任何 key。** GitHub Actions = cron。与 SCOPE（个人、不分发）一致。
...
| ID | 需求 |
|---|---|
| NFR-1 | **零常驻后端**：无 server、无托管 DB、client 不暴露 key（§5.2） |
...
| **M1** | Leaders + digest：bounded top-N board + 旋钮 + d/d + 每日 email digest | compute, web | AC-M1 |
...
- **AC-M1**：bounded top-N board 渲染；旋钮改 k 时排序实时变化；d/d（day-over-day）标注；email digest 能发出。
...
- spine：**1 个 per-stock 引擎 → 5 surface → 2 scale → 零持久后端**。
...
- 客户端框架、duckdb-wasm 接法、shard 切分粒度（§5）。
- email digest 的实现方式（M1）。
