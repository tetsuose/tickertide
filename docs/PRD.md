...
- 估值：price ÷ trailing-4Q 日频、**formal-filing PIT**（分母只用正式 SEC filing、ASOF 键 = `effective_eod_date`、`valuation_basis='formal_filing_pit'`；§10.5）、`E≤0→n.m.` 退 P/S、无 forward、百分位用 **common-vintage**（当前 fresh-cohort v1）；**Ocean 横轴用原始 P/S（log），非 percentile**（§9.2）。
